...
CLASSIFICATION_MODEL_PATH = r"new_models\classify_trin9.pt"
...
RECIPIENT_EMAILS = [
    "attakorn.chantarasuksun@autoliv.com",
    "paknarit.petsrichot@autoliv.com",
...
def setup_logging():
    ...
    log_filename = os.path.join(log_dir, f"ppe_system_{datetime.now().strftime('%Y%m%d')}.log")
    ...
    logger.info("=" * 80)
    logger.info("PPE Detection System Started")
...
def insert_ng_ticket(camera_id: int, location: str, rule_id: int, severity: str = "High"):
    try:
        ...
        if not conn:
            logging.error("insert_ng_ticket: Cannot connect to database")
            return None
        ...
        VALUES (?, ?, ?, ?, 'New', 'AI System', ?);
        """

        logging.info(f"Creating NG Ticket: Camera={camera_id}, Location={location}, RuleID={rule_id}, Severity={severity}, Time={current_time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        cursor.execute(query, (current_time, location, rule_id, severity, current_time))
        row = cursor.fetchone()
        
        if not row:
            conn.rollback()
            logging.error("insert_ng_ticket: INSERT succeeded but no TicketID returned")
            return None
        ...
        logging.info(f"✅ NG Ticket created: ID={ticket_id}, Camera={camera_id}, Location={location}, RuleID={rule_id}, Time={current_time.strftime('%Y-%m-%d %H:%M:%S')}")
        return ticket_id

    except Exception as e:
        logging.error(f"Error inserting NG ticket: {e}", exc_info=True)
        try:
            conn.rollback()
            cursor.close()
            conn.close()
...
def insert_ng_evidence(ticket_id: int, file_path: str, file_type: str = "Image"):
    try:
        if not ticket_id:
            logging.error(f"insert_ng_evidence: Invalid ticket_id={ticket_id}")
            return False
        
        if not file_path or not os.path.exists(file_path):
            logging.error(f"insert_ng_evidence: File not found: {file_path}")
            return False
        
        conn = get_db_connection()
        if not conn:
            logging.error("insert_ng_evidence: Cannot connect to database")
            return False
        ...
        logging.info(f"Inserting evidence: TicketID={ticket_id}, File={os.path.basename(file_path)}, Type={file_type}, Time={current_time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        cursor.execute(query, (ticket_id, file_path, file_type, current_time))
        conn.commit()
        
        cursor.close()
        conn.close()
        
        logging.info(f"✅ Evidence saved: TicketID={ticket_id}, File={os.path.basename(file_path)}")
        return True
        
    except Exception as e:
        logging.error(f"Error inserting evidence: {e}", exc_info=True)
        try:
            conn.rollback()
            cursor.close()
            conn.close()
...
def update_ticket_status(ticket_id: int, status: str, notified_email: str = None):
    try:
        ...
        if notified_email:
            query = """
            UPDATE ppe_NGTicket
            SET Status = ?, NotifiedEmail = ?, UpdatedDate = ?
            WHERE TicketID = ?
            """
            cursor.execute(query, (status, notified_email, current_time, ticket_id))
        else:
            query = """
            UPDATE ppe_NGTicket
            SET Status = ?, UpdatedDate = ?
            WHERE TicketID = ?
            """
            cursor.execute(query, (status, current_time, ticket_id))
        
        conn.commit()
        cursor.close()
        conn.close()
        
        logging.info(f"✅ Ticket {ticket_id} updated to status: {status} at {current_time.strftime('%Y-%m-%d %H:%M:%S')}")
        return True
